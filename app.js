...
app.get("/pt.html",async function(req,res){
  let profits =await Profit.find();
  //console.log(products);
  res.render("pt",{profits:profits});
});
...
var message="Total Profit of "+x+" day is "+sum;
...
var message="Total Profit of "+y+ " month is "+sum;
res.render("profits",{message:message});
});
...
var message="Total Profit of "+y+" year is "+sum;
res.render("profits",{message:message});
});
...
app.post("/order.html",async function(req,res){
  ...
  if (doc == null) {
    ...
    if(doc.quant<quantity){
      res.render("stocknot",{message:"Quantity is less than demanded, current available mobiles are "+doc.quant});
    }else{
      const doc = await Product.findOne(fil);
  ...
  res.render("profits",{message:"Order completed successfully!!"});
});
